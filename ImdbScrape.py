...
def getmovies(m):
    ...
    imgs = [re.search('src="(.+?)._', str(text)).group(1)
            for text in movies[0::2]]
    titles = [re.search('>(.+?)<', str(text)).group(1)
              for text in movies[1::2]]
    return urls, imgs, titles


def user_review(url, n):
    url = 'https://www.imdb.com/' + url + \
        'reviews?spoiler=hide&sort=helpfulnessScore&dir=asc&ratingFilter=' + \
        str(n)
...
def create_csv(urls, i, m):
    ...
    df = pd.DataFrame(list(zip(review, rate)),
